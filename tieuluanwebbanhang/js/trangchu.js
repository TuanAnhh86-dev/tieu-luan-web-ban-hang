window.onload = function () {
	...
	var tags = ["Sữa", "Thịt", "Nước lau sàn", "Trái cây"];
	...
	if (filters.length) { // có filter
		...
		addKhungSanPham('ƯU ĐÃI HỘI VIÊN', green, ['promo=giareonline', 'sort=rateCount-decrease'], soLuong, div);
		addKhungSanPham('SỮA CÁC LOẠI', blue, ['promo=moiramat', 'sort=rateCount-decrease'], soLuong, div);
		addKhungSanPham('TRÁI CÂY', yellow_red, ['promo=tragop', 'sort=rateCount-decrease'], soLuong, div);
		addKhungSanPham('THỊT - HẢI SẢN TƯƠI', green, ['promo=giareonline', 'sort=rateCount-decrease'], soLuong, div);
		addKhungSanPham('CHẤT TẨY RỬA', yellow_red, ['promo=giamgia'], soLuong, div);
...
function priceToString(min, max) {
	if (min == 0) return 'Dưới ' + max / 1E6 + ' chục nghìn';
	if (max == 0) return 'Trên ' + min / 1E6 + ' chục nghìn';
	return 'Từ ' + min / 1E6 + ' - ' + max / 1E6 + ' chục nghìn';
}
